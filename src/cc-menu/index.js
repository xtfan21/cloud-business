...
import { Icon, Menu } from 'cloud-react';
...
export default class CcMenu extends PureComponent {
    ...
    handleShopSearch = keyword => {
        const newShopSource = this.props.shopSource.reduce((acc, plat) => {
            const { children } = plat;
            const newChildren = children.filter(shop => shop.title.includes(keyword));

            if (!Array.isArray(children) || children.length === 0 || newChildren.length === 0) {
                return acc;
            }
            ...
            <div className="menu-collapsed-bar">
...

                {
                    ...
                    <div className="collapsed-logo-wrapper" onClick={this.toggleSelector}>
                        <Icon type="shop" className="collapsed-logo"></Icon>
                    </div>
                }
            </div>
        );
